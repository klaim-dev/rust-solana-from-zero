use day28::app::service::AppCommand;
use day28::cli::{parse_args, UsageError};
...
fn add_order_ok() {
    let cli =
        parse_args(args(&["add-order", "--id", "1", "--customer", "alice"])).expect("parse args");
...
#[test]
fn list_customer_ok() {
    let cli = parse_args(args(&["list", "--customer", "alice"])).expect("parse args");
...
fn add_item_invalid_qty_int() {
    let result = parse_args(args(&["add-item", "--id", "1", "--sku", "a", "--qty", "x"]));
    match result {
        Err(UsageError::InvalidInt {
            flag: "--qty",
            input,
        }) if input == "x" => {}
        _ => panic!("unexpected result"),
...
fn list_id_is_unexpected() {
    match parse_args(args(&["list", "--id", "1"])) {
        Err(UsageError::UnexpectedFlagForCommand { cmd, flag: "--id" }) if cmd == "list" => {}
        _ => panic!("unexpected result"),
    }
}
...
fn add_order_invalid_id_value() {
    let cli = parse_args(args(&["add-order", "--id", "0", "--customer", "a"])).expect("parse args");
...
fn add_item_invalid_sku_value() {
    let cli = parse_args(args(&["add-item", "--id", "1", "--sku", "", "--qty", "1"]))
        .expect("parse args");
    match cli.into_app_command() {
        Err(UsageError::InvalidValue {
            flag: "--sku",
