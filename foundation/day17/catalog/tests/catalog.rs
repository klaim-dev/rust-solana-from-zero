...
use catalog::domain::item::{Category, CreateItem, Filter, Sku, UpdateItem};
...
fn test_update_item_sku_rebinding() {
    ...
    assert_eq!(catalog.get_by_sku(&sku("SKU001_NEW")).unwrap().id(), id1);

    // Verify item 2 is still accessible by its original SKU
    assert_eq!(catalog.get_by_sku(&sku("SKU002")).unwrap().id(), id2);
}
...
fn test_update_item_sku_collision_error() {
    ...
    assert_eq!(
        result,
        Err(CatalogError::SkuCollision { sku: sku("sku002") })
    );
    ...
    assert_eq!(
        catalog.get_by_id(&id1).unwrap().name(),
        original_item1.name()
    );
    assert_eq!(
        catalog.get_by_id(&id2).unwrap().name(),
        original_item2.name()
    );
}
...
fn test_list_items_anti_n1_architecture() {
    ...
    assert!(
        items
            .iter()
            .all(|item| { item.price_cents() >= 200 && item.price_cents() <= 800 })
    );
...
fn test_duplicate_sku_is_rejected() {
    ...
    assert_eq!(result2, Err(CatalogError::DuplicateSku { sku: sku("abc") }));
}
