...
use crate::defi::validate::{self, validate_transfer};
...
let len = xs.len();
if i >= len || j >= len {
    Err(TransferError::IndexOutOfBounds { len: len, i: i, j: j })?
}
if i == j {
    Err(TransferError::SameIndex)?
}
...
let to_new = to.amount.0.checked_add(amount.0).ok_or(TransferError::OverflowToBalance)?;
