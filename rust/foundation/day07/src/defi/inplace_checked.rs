...
fn get_two_mut_accouts<'a>(accounts: &'a mut [TokenAccount], i: usize, j: usize) -> Option<(&'a mut TokenAccount, &'a mut TokenAccount)> {
...
pub fn apply_transfer_by_index_checked(accounts: &mut [TokenAccount], i: usize, j: usize, amount: Lamports, mint: Pubkey) -> Result<(), TransferError> {
    let len = accounts.len();
    println!("{}", len);
    let (from, to) = get_two_mut_accouts(accounts, i, j).ok_or_else(|| TransferError::IndexOutOfBounds { len: len, i: i, j: j })?;
